...
const documentBaseURI = `${ baseURI }/resources/docs/`
const annotationBaseURI = `${ baseURI }/resources/annotations/`
...
const run = async():Promise<any> => {
    ...
    const apollo = new ApolloServer({
        schema,
        dataSources: () => ({
            annotations: new AnnotationStore({annotationsCollection, annotationBaseURI})
        })
    });
    await apollo.start();
    apollo.applyMiddleware({app});

    const documentStore = new DocumentStore({documentsCollection, annotationsCollection, documentBaseURI});
